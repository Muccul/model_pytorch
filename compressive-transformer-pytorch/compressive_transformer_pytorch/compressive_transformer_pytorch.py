...
class SelfAttention(nn.Module):
    ...
    def forward(self, x, memories = None, pos_emb = None, input_mask = None, calc_memory = True, **kwargs):
        #  x [b,len_x,d]   pos_emb[h,len_pos,h_d]
        ...
        mem = default(mem, init_empty_mem)   # [b,len_mem,d]
        cmem = default(cmem, init_empty_mem)  # [b,len_cmem,d]

        mem_len = mem.shape[1]
        cmem_len = cmem.shape[1]

        q = self.to_q(x)  # [b,len_x,d]

        kv_input = torch.cat((cmem, mem, x), dim=1)  # [b,len_cmem+mem+x,d]
        kv_len = kv_input.shape[1]  # len_cmem+mem+x
        ...
        dots = torch.einsum('bhid,bhjd->bhij', q, k) * self.scale   # [b,h,len_x,len_cmem+mem+x]
        mask_value = max_neg_value(dots)

        if pos_emb is not None:
            pos_emb = pos_emb[:, -kv_len:].type(q.dtype)  # [h,len_cmem+mem+x,h_d]
            pos_dots = torch.einsum('bhid,hjd->bhij', q, pos_emb) * self.scale  # [b,h,len_x,len_cmem+mem+x]
...
class CompressiveTransformer(nn.Module):
    def __init__(self, num_tokens, dim, seq_len, depth, emb_dim = None, memory_layers = None, mem_len = None, cmem_len = None, cmem_ratio = 4, heads = 8, gru_gated_residual = True, mogrify_gru = False, attn_dropout = 0., ff_glu = False, ff_dropout = 0., attn_layer_dropout = 0., reconstruction_attn_dropout = 0., reconstruction_loss_weight = 1., one_kv_head = False):
        ...
        self.ff_layers = nn.ModuleList(
            [wrapper(PreNorm(dim, FeedForward(dim, dropout=ff_dropout, glu=ff_glu))) for _ in range(depth)])
